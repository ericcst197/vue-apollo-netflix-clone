...
const { id, type = 'text', label = "", placeholder , helperText = "", required, disabled = false, warn = false, modelValue, maxLength = null, theme = 'dark' } = defineProps<Props>()
...
<template>
    <div class="relative flex flex-col flex-1 pb-4">
        <div class="flex flex-row items-center rounded text-white bg-zinc-900" :class="themeClassMap[theme]">
            ...
            <div class="relative flex-auto">
                <input :id="id" :value="modelValue" @input="handleInput" :type="inputType" @focusin="onFocus" @focusout="onUnFocus"
                    ...
                    class="block rounded-t-lg px-2.5 pb-2.5 pt-5 w-full text-sm text-white bg-transparent appearance-none focus:outline-none focus:ring-0 focus:bg-[#454545] peer"
            ...
            <button v-if="type == 'password'" class="absolute right-2 mx-3" :class="{'right-10': warn}"
        ...
        <p v-if="helperText" :class="{ 'text-orange-500': warn, 'text-gray-500': !warn }" class="text-sm mt-1.5">
            {{ helperText }}</p>
    </div>
</template>
